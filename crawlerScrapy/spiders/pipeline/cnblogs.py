...
class CnblogsPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        # sql语句
        insert_sql = """
        insert into cnblogs(title, real_desc, pub_time, author, url) VALUES (%s,%s,%s,%s,%s)
        """

        try:
            # 执行插入数据到数据库操作
            self.cursor.execute(
                insert_sql, ('{}'.format(item['title']), item['real_desc'], item['pub_time'], item['author'], item['url']))
